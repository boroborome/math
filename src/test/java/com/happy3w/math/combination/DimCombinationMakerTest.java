...
import com.happy3w.java.ext.MapUtils;
...
public class DimCombinationMakerTest {
    ...
    private MyConfig createConfig(DimCombineDetail<String, String> detail, Map<String, MyConfig> configMap) {
        ...
        MyConfig config = configMap.computeIfAbsent(key, k -> new MyConfig(detail.getNormalResult()));
        ...
        for (DimCombineDetail<String, String> overedDetail : detail.getOveredCombines()) {
            ...
            config.getSubConfigs().add(MapUtils.safeRead(configMap, () -> new MyConfig(overedDetail.getNormalResult()), newKey));
        }
        return config;
    }
